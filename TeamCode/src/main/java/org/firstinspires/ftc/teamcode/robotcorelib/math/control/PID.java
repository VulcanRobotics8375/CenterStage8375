...
public class PID {
    ...
    public double getOutput(double actual, double setpoint){
        ...
        if (!firstRun && (time - lastTime) < sampleTime) {
            return lastOutput;
        }
        ...
        // Make F term 0 if setpoint 0
        Foutput = setpoint == 0 ? 0 : F;
    ...
    private boolean bounded(double value, double min, double max){
        ...
        // `bounded(Range.clip(0,0,1),0,1)` will return false.
